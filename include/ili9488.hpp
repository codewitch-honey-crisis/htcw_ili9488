...
namespace arduino {
    ...
    struct ili9488 final {
        constexpr static const bool is_18bit = Bus::type!=tft_io_type::parallel8;
        ...
        ili9488() : m_initialized(false), m_dma_initialized(false), m_in_batch(false) {
        }
        ~ili9488() {
        ...
        bool initialize() {
            if(!m_initialized) {
                if(driver::initialize()) {
                    ...
                    driver::send_command(0XE1); // Negative Gamma Control
                    driver::send_data8(0x00);
                    driver::send_data8(0x16);
                    driver::send_data8(0x19);
                    driver::send_data8(0x03);
                    driver::send_data8(0x0F);
                    driver::send_data8(0x05);
                    driver::send_data8(0x32);
                    driver::send_data8(0x45);
                    driver::send_data8(0x46);
                    driver::send_data8(0x04);
                    driver::send_data8(0x0E);
                    driver::send_data8(0x0D);
                    driver::send_data8(0x35);
                    driver::send_data8(0x37);
                    driver::send_data8(0x0F);

                    driver::send_command(0XC0); // Power Control 1
                    driver::send_data8(0x17);
                    driver::send_data8(0x15);

                    driver::send_command(0xC1); // Power Control 2
                    driver::send_data8(0x41);

                    driver::send_command(0xC5); // VCOM Control
                    driver::send_data8(0x00);
                    driver::send_data8(0x12);
                    driver::send_data8(0x80);

                    driver::send_command(0x36); // Memory Access Control
                    driver::send_data8(0x48);          // MX, BGR

                    driver::send_command(0x3A); // Pixel Interface Format
                    if(!is_18bit) {
                        driver::send_data8(0x55);  // 16 bit colour for parallel
                    } else {
                        driver::send_data8(0x66);  // 18 bit colour for SPI
                    }

                    driver::send_command(0xB0); // Interface Mode Control
                    driver::send_data8(0x00);

                    driver::send_command(0xB1); // Frame Rate Control
                    driver::send_data8(0xA0);

                    driver::send_command(0xB4); // Display Inversion Control
                    driver::send_data8(0x02);

                    driver::send_command(0xB6); // Display Function Control
                    driver::send_data8(0x02);
                    driver::send_data8(0x02);
                    driver::send_data8(0x3B);

                    driver::send_command(0xB7); // Entry Mode Set
                    driver::send_data8(0xC6);

                    driver::send_command(0xF7); // Adjust Control 3
                    driver::send_data8(0xA9);
                    driver::send_data8(0x51);
        ...
        inline gfx::size16 dimensions() const {
            return rotation&1?gfx::size16(480, 320):gfx::size16(320, 480);
        }
        ...
        gfx::gfx_result point(gfx::point16 location, pixel_type color) {
            ...
            if(!is_18bit) {
                ...
                uint8_t r = (color.native_value & 0xF800)>>8;
                uint8_t g = (color.native_value & 0x07E0)>>3;
                uint8_t b = (color.native_value & 0x001F)<<3;
        ...
        gfx::gfx_result fill(const gfx::rect16& bounds, pixel_type color) {
            ...
            if(!bounds.intersects(this->bounds())) return gfx::gfx_result::success;
            const gfx::rect16 rct = bounds.normalize().crop(this->bounds());
            bus::begin_write();
            bus::begin_transaction();
            set_window(rct);
            if(!is_18bit) {
                bus::write_raw16_repeat(color.native_value,(rct.x2-rct.x1+1)*(rct.y2-rct.y1+1));
            } else {
                uint32_t count = (rct.x2-rct.x1+1)*(rct.y2-rct.y1+1);
                // Split out the colours
                uint8_t r = (color.native_value & 0xF800)>>8;
                uint8_t g = (color.native_value & 0x07E0)>>3;
                uint8_t b = (color.native_value & 0x001F)<<3;
    ...
    private:
        ...
        gfx::gfx_result copy_from_impl(const gfx::rect16& src_rect,const Source& src,gfx::point16 location,bool async) {
            ...
            return copy_from_helper<Source,gfx::helpers::is_same<pixel_type,typename Source::pixel_type>::value && (Source::caps::blt && !is_18bit)>
    ...
    template<int8_t PinDC, 
        ...
        unsigned int ReadSpeedPercent> int ili9488<PinDC, 
                                                PinRst, 
                                                PinBL, 
                                                Bus, 
                                                Rotation, 
